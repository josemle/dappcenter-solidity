...
contract DAO
{
  ...
  function executeProposal(uint proposalId) onlyMembers public
  {
    ...
    Proposal storage proposal = idToProposal[proposalId];
    // TODO: this is throwing a compile error ATM
  ...
  function addMember(address payable _address, uint _weight) onlyApprovedProposals public
  {
    _withdrawl();
    _addMember(_address, _weight);
  }

  function _addMember(address payable _address, uint _weight) internal
  {
  ...
  function swapMember(address _originalAddress, address payable _newAddress) onlyApprovedProposals public
  {
  ...
  function setMemberWeights(address[] memory _addresses, uint[] memory _weights) onlyApprovedProposals public
  {
  ...
  function send(address payable _to, uint _amount) onlyApprovedProposals public
  {
  ...
  function withdrawlERC20(address _tokenAddress) onlyMembers public
  {
    ...
    for(i = 0; i < members.length; i++)
    {
      Member memory member = members[i];
      uint share = balance.mul(member.weight).div(totalWeight);
      if(erc20.transfer(member.memberAddress, share))
      {
        totalAmount += share;
      }
    }
